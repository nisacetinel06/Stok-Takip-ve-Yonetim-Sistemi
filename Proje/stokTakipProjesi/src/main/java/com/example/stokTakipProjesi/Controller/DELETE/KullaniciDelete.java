...
public class KullaniciDelete implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("DELETE".equalsIgnoreCase(exchange.getRequestMethod())) {
            ...
            if (jsonObject.has("id")) {
                kullaniciId = jsonObject.get("id").getAsInt();
            }
            ...
            try {
                ...
                if (check > 0) {
                    jsonResponse = "{\"status\": \"success\", \"message\": \"Kullanıcı başarıyla silindi!\"}";
                } else {
                    jsonResponse = "{\"status\": \"failure\", \"message\": \"Kullanıcı bulunamadı veya silinemedi!\"}";
                }
