...
public class TedarikciDelete implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("DELETE".equalsIgnoreCase(exchange.getRequestMethod())) {
            ...
            if (jsonObject.has("id")) {
                tedarikciId = jsonObject.get("id").getAsInt();
            }
            ...
            try {
                ...
                String sql = "DELETE FROM tedarikciler WHERE id=?";
                PreparedStatement pstmt = connection.prepareStatement(sql);
                pstmt.setInt(1, tedarikciId);
                int check = pstmt.executeUpdate();

                System.out.println("Delete Result: " + check);
                if (check > 0) {
                    jsonResponse = "{\"status\": \"success\", \"message\": \"Tedarikçi başarıyla silindi!\"}";
                } else {
                    jsonResponse = "{\"status\": \"failure\", \"message\": \"Tedarikçi bulunamadı veya silinemedi!\"}";
                }
